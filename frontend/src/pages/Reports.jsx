...
const MICRO_KEYS = [
  ...
  (r) => r.carbidePercentMin ?? r.carbidePercent, (r) => r.carbidePercentMax ?? r.carbidePercent,
  (r) => r.sizeMin ?? r.size, (r) => r.sizeMax ?? r.size, 'remarks'
];
                ...
                {results.microStructure.length === 0
                  ...
                  : results.microStructure.map((r, idx) => (
                    <tr key={`${r.id}-${r.location}-${idx}`}>
                      ...
                      <td>{dash(r.pearlitePercentMin ?? r.pearlitePercent)}</td><td>{dash(r.pearlitePercentMax ?? r.pearlitePercent)}</td>
                      <td>{dash(r.carbidePercentMin ?? r.carbidePercent)}</td><td>{dash(r.carbidePercentMax ?? r.carbidePercent)}</td>
                      <td>{dash(r.sizeMin ?? r.size)}</td><td>{dash(r.sizeMax ?? r.size)}</td>
                      <td style={{ maxWidth: '200px', whiteSpace: 'normal' }}>{dash(r.remarks)}</td>
                    </tr>
                  ))
                }
                ...
                {results.impactTest.length === 0
                  ...
                  : results.impactTest.map((r, idx) => (
                    <tr key={`${r.id}-${r.location}-${r.notchType}-${idx}`}>
                      ...
                      <td>{r.location || r.mechLocation || '—'}</td><td>{r.disa || '—'}</td><td>{r.notchType || '—'}</td>
