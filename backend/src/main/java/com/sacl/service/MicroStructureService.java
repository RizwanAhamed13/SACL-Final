...
public class MicroStructureService {
    ...
    public int approveAll(String approvedBy) {
        ...
        List<MicroStructureAnalysis> pending = repository.findByStatus(RecordStatus.HOF_APPROVED);
        pending.forEach(r -> {
            r.setStatus(RecordStatus.HOD_APPROVED);
            r.setHodApprovedBy(approvedBy);
        });
        repository.saveAll(pending);
        log.info("Micro Structure bulk approved {} records by {}", pending.size(), approvedBy);
        return pending.size();
    }
